...
public class OrderDAOImpl implements OrderDAO {
    ...
    private void sqlCommand(Order obj, String sql) {
        try {
            ...
            preparedStatement.setObject(2, obj.getClient().getId());
            preparedStatement.setObject(3, obj.getMechanic().getId());
    ...
    public List<Order> findAllByClientOrStatusOrDescription(Client client, Status status, String description) {
        ...
        try {
            ...
            if (client != null) {
                preparedStatement.setObject(count, client.getId());
                count++;
            }
            if (status != null) {
    ...
    public List<Order> findAllByMechanicAndStatus(Mechanic mechanic, Status status) {
        ...
        PreparedStatement preparedStatement = BaseConnect.preparedStatement(sql);
        try {
            preparedStatement.setObject(1, mechanic.getId());
            ...
            while (resultSet.next()) {
                orderList.add(
                        new Order(
                                ...
                                clientObject.getById((UUID) resultSet.getObject("client")),
                                mechanicObjectDAO.getById((UUID) resultSet.getObject("mechanic")),
